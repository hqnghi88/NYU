...
global {
	...
	init {
		...
		if (empty(schools)) { 
			...
			ask 2 among building { type <- "school"; color <- #cyan; }
			schools <- building where (each.type = "school");
		}
		if (empty(hospitals)) {
			write "No hospitals found in OSM. Randomly assigning 1.";
			ask 1 among building { type <- "hospital"; color <- #magenta; }
			hospitals <- building where (each.type = "hospital");
		}
		...
		if (scenario_buses) {
			...
			ask hospitals { stop_locations <+ any_location_in(shape); }
			ask 3 among building { stop_locations <+ any_location_in(shape); }
			
			loop loc over: stop_locations {
				create bus_stop { location <- loc; }
			}
	...
	reflex decide_target when: target = nil {
		if (is_school_run) {
			...
			if (scenario_stagger_school) {
				...
				if (flip(0.02)) { target <- any_location_in(target_building); }
			} else {
				// Everyone rushes (higher prob)
				if (flip(0.1)) { target <- any_location_in(target_building); }
			}
		} else {
